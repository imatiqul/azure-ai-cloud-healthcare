...
/**
 * Hook that streams a static string word-by-word on mount, simulating live AI output.
 * Safe to use in lists — streaming runs once on mount and does not restart on re-renders.
 *
 * @param text  Full text to reveal
 * @param msPerToken Delay between tokens in ms (default 28)
 */
export function useStreamText(text: string, msPerToken = 28) {
  const [displayed, setDisplayed] = useState('');
  const [streaming, setStreaming] = useState(false);
  const [done, setDone] = useState(false);
  // Stable ref so the effect only runs once per mount, regardless of text prop updates
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      i++;
      id = setTimeout(tick, msPerToken);
    };
    let id = setTimeout(tick, msPerToken);
    return () => clearTimeout(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // intentionally empty — stream once on mount
